...
//       }
//       if (candidatdate_id.resize(G.n);
//         std::iota(cane_id.empty()) {
//         candididate_id.begin(), candidate_id.end(), 0);
//       }
//       for (auto [phi, val]: J) {
...
//       }
    ...
    for (auto &[phi, val] : J) {
        ...
        // inserting the new vertex at p
        auto psi = phi;
        psi.insert(psi.begin() + p, 0);
        for (int a : candidateTargets) {
            psi[p] = a;
            I[psi] += val;   // accumulate counts
        }
    }
